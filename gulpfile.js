...
var REPO = 'git@github.com:mihaislobozeanu/pdf.js.git';
var DIST_REPO_URL = 'https://github.com/mihaislobozeanu/embed-pdfjs-dist.git';
...
gulp.task('generic', ['buildnumber', 'locale'], function () {
	...
	return merge([
		...
		createEmbeddedWebBundle(defines).pipe(gulp.dest(GENERIC_DIR + 'web')),
...
gulp.task('minified-pre', ['buildnumber', 'locale'], function () {
	...
	return merge([
		...
		createEmbeddedWebBundle(defines).pipe(gulp.dest(MINIFIED_DIR + 'web')),
		gulp.src(COMMON_WEB_FILES, { base: 'web/', })
...
gulp.task('dist-pre',
	...
	function () {
		...
		var DIST_NAME = 'embed-pdfjs-dist';
		var DIST_DESCRIPTION = 'Generic build of Mozilla\'s PDF.js library. An embedded viewer provided.';
		var DIST_KEYWORDS = ['Mozilla', 'pdf', 'pdf.js'];
		var DIST_HOMEPAGE = 'https://github.com/mihaislobozeanu/embed-pdfjs-dist';
		var DIST_BUGS_URL = 'https://github.com/mihaislobozeanu/embed-pdfjs-dist/issues';
